...
**DoorObstacle** (`objects/obstacles/door_obstacle.gd` + `DoorObstacle.tscn`, `class_name DoorObstacle extends FseObstacle`) — a key-gated blast door for gating rail progress. Inherits `BaseObstacle` (so it has the shared `ContactDamage`, set to **100 dmg**), is **indestructible** (`is_destructible = false`, `homing_eligible = false`) — you can't shoot the door open. It exposes an Inspector `key_blasts: Array[Node]` (drag in the **KeyBlast** enemy instances placed nearby) and `keys_required: int` (0 = all assigned). It connects to each key's `died` signal; once the threshold is destroyed it **opens**: the two `LeftPanel`/`RightPanel` meshes tween apart (`panel_slide_distance` / `open_duration`) and the `ContactDamage` is switched off (`set_active(false)`) so the player passes through unharmed. Fail to clear the keys and you slam into the closed door for 100 and die. **KeyBlast** (`objects/enemy/blast/KeyBlast.tscn` + `key_blast.tres`) is a plain stationary `FseEnemy` target (no movement/weapon/blast — just shoot it to raise its `died`).
