...
- **`main.tscn`** — the playable scene, at the **project root**. `levels/` holds the rail (`rail_follower.gd`), `main.gd`, the door-gated exit routes (`exit_rail.gd` + `ExitRail.tscn`), and overworld pieces.
- **`objects/`** — game objects: `objects/components/` (shared behavior components, used by enemies **and** the player), `objects/enemy/` (FseDestructible/FseEnemy bases, concrete enemies, `enemy/blast/`, movement patterns, `*Data` resources), `objects/obstacles/`, `objects/weapons/`, `objects/player/` (the player rig — the `mecha_player.gd` hub + `MechaPlayer.tscn` + control-scheme components like `aim_led_scheme.gd`), and `objects/triggers/` (the legacy dialogue/trigger system).
...
- **`ui/`** — HUD / menus, flat (`ui/CombatUI.tscn`, `ui/combat_ui.gd`, the `StartScreen.tscn`/`start_screen.gd` menu, plus the legacy `DialogueBox`/`PromptBox`).
...
The rig is driven forward by **`levels/rail_follower.gd`** on the `PathFollow3D`: each frame it advances `progress` by `speed` (world units/sec, default ~20), with `loop` on so it wraps at the curve's end. **`ToggleBrake`** flips a `braked` flag — press to stop, press again to resume. (This replaced the old `AnimationPlayer/MoveF` track that animated `progress_ratio` 0→1; the AnimationPlayer stays in `main.tscn` but no longer autoplays.) The ship mesh plays a constant vertical bob — a subtle sway in flight (`move_bob_*`, default 0.05u) that crossfades into a deeper floating-in-midair hover while braked (`brake_bob_*`, default 0.15u) — `mecha_player.gd` reads the rail's `braked` flag (via `rail_follower_path` / nearest `PathFollow3D` ancestor) and blends the sine offset on the `mecha-frame` mesh with a continuously-running phase, so the transition never pops ("Bob" knobs).

**Hub + swappable control scheme:** `mecha_player.gd` is a **hub** owning the scheme-independent identity — HP/damage/death, the weapon mount + firing loop, homing acquisition, the rail-coupled bob, and mouse-capture policy. "How the ship moves & where it aims" lives in a **control-scheme child component** the hub ticks each physics frame (schemes do *not* self-process — their internal order is load-bearing) and consumes only via ports: `update(delta)`, `set_active(bool)`, `get_aim_world_point()` (weapon convergence), `get_reticle_screen_position()` (HUD + homing anchor), `is_evading()` (fire gate), `is_aiming()`. Swapping schemes (e.g. a future ship-led Star Fox SNES scheme with in-world 3D aim indicators and its own Camera React knobs) touches only the scheme child — weapons, homing, HUD, and damage flow stay identical. The scheme is referenced duck-typed (first child exposing the ports).

The current scheme is **`AimLedScheme`** (`objects/player/aim_led_scheme.gd`, the `AimLedScheme` node in `MechaPlayer.tscn`). Its internal per-frame order is **evade → aim → camera → ship** (the ship reads the camera's banked transform, so the camera must rotate first); the AimTarget node is this scheme's private cursor home (hub resolves it from the rig and injects it via `setup()`):
...
Inspector-tunable knobs are deliberately dense and live under `@export_category` blocks, split by ownership: **the `AimLedScheme` node in `MechaPlayer.tscn`** holds Movement, Aiming, Ship Follow, Camera React, and Evade; **the hub (`Navigator`)** holds Combat (weapon/hp/homing), Bob, Input, and References (the hit flash/shake knobs live on the `HitReactComponent`). The user iterates heavily in the Inspector — tune scheme feel by opening `MechaPlayer.tscn` and selecting the scheme node.
...
Obstacles (`objects/obstacles/`): **BobBlock** (MovementComponent + BobMovement), **AnimObstacle** (AnimationDriver + keyframed clip), **StationaryTurret** (a stationary `FseObstacle` with a `TurretEmitter` firing curve-following `TurretBolt`s), **DoorObstacle** (see below). `TurretBolt` (`objects/weapons/enemy/TurretBolt.tscn`) is destructible but sets `homing_eligible = false` so the player can't lock onto incoming fire.

**DoorObstacle** (`objects/obstacles/door_obstacle.gd` + `DoorObstacle.tscn`, `class_name DoorObstacle extends FseObstacle`) — a key-gated blast door for gating rail progress. Inherits `BaseObstacle` (so it has the shared `ContactDamage`, set to **100 dmg**), is **indestructible** (`is_destructible = false`, `homing_eligible = false`) — you can't shoot the door open. It exposes an Inspector `key_blasts: Array[Node]` (drag in the **KeyBlast** enemy instances placed nearby) and `keys_required: int`. Two open modes: **key-gated** (`keys_required > 0` *and* keys assigned) — connects to each key's `died` signal and opens once `keys_required` of them are destroyed; **auto-open** (`keys_required = 0` *or* no keys assigned) — opens when the player comes within `auto_open_distance` (~20u), for tutorial stages / cinematic doors. On open the two `LeftPanel`/`RightPanel` meshes tween apart (`panel_slide_distance` / `open_duration`) and the `ContactDamage` is switched off (`set_active(false)`) so the player passes through unharmed. Fail to clear a key-gated door and you slam into it for 100 and die. **KeyBlast** (`objects/enemy/blast/KeyBlast.tscn` + `key_blast.tres`) is a plain stationary `FseEnemy` target (no movement/weapon/blast — just shoot it to raise its `died`).

A door can also own an **exit route**: assign an **`FseExitRail`** (`levels/exit_rail.gd` + instanceable `levels/ExitRail.tscn`) to the door's `exit_rail` export, the same drag-in pattern as `key_blasts`. `FseExitRail` extends the NurbsPath3D script (by **path**, dodging the class-cache lag; the gizmo matches subclasses via the base-script chain, so control-point authoring works on it). Flow: door opens → calls the rail's `activate()` → the rail watches the player rig and **arms** when the rig comes within `arm_radius` (~35u) of its start point, then **captures** the rig — reparents the `RailFollow` `PathFollow3D` onto itself at the matching closest-offset, `loop = false`, with `PlayerRoot` absorbing the residual snap (`capture_smooth_time`). The arm-then-cross gate matters because exit routes may overlap the loop further along — without it a door opening mid-overlap would capture immediately. When the rig reaches the rail end it emits `Events.exit_rail_completed` — `main.gd` listens and runs **victory**: the rig keeps flying along the end tangent while a `FadeLayer/FadeRect` tweens to black (`victory_fade_duration` / `victory_reset_delay`), then the game returns to the **start screen** (`ui/StartScreen.tscn` — a minimal Play/Quit menu; Play reloads `main.tscn` fresh, and it releases the mouse capture on entry). Death does the same after `death_reset_delay`. `main.tscn` is still the editor's `run/main_scene`; flip that to the start screen when packaging builds. Multiple doors with their own exit rails are supported by construction (per-door Inspector wiring); currently any completed exit rail triggers victory.
...
- During verification, enemies kill the player fast, which fires `Events.player_killed` → after `death_reset_delay` the scene switches to the start screen, invalidating any node references an eval is holding. For sustained inspection, set the player's `hp`/`max_hp` huge and halt the rail in a `game_eval` first (set the `PathFollow3D`'s `braked = true`, or `speed = 0`).
